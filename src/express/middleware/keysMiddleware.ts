...
export default function keysMiddleware(request: express.Request, response: express.Response, next: () => void) {
  const body = request.body;
  let sender = null;

  if (body.hasOwnProperty("gasPayer") &&
    ...
    const pair = createPair({ toSS58, type: 'sr25519' }, { publicKey, secretKey: privateKey });
